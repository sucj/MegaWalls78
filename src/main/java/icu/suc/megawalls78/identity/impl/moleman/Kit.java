...
public final class Kit extends icu.suc.megawalls78.identity.Kit {
    ...
    @Override
    protected void init() {
        addTool(DIAMOND_SHOVEL).addEnchantment(EFFICIENCY, 2, true).addEnchantment(SHARPNESS, 2, true);
        addEnderChest();
        addPickaxe();
        addTool(BOW);
        addItem(COOKED_BEEF, 3);
        addHealingPotion(2, 3).addCustomEffect(new PotionEffect(REGENERATION, 0, 4), false);
        addCompass();
